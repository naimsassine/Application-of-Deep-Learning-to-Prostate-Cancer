...
def training_model_with_da():
    ...
    prepare_data_lesion_dc()

    X_train, y_train, X_val, y_val = load_only_training(test_type="detection_and_classification")
    ...
    # model.load_weights('data/detection_and_classification/classifi_weights.h5')

    model.summary()
    model_checkpoint = ModelCheckpoint(
        'data/detection_and_classification/classifi_weights.h5', monitor='val_loss', save_best_only=True)

    call_backs = [model_checkpoint]
    #call_backs.append(EarlyStopping(monitor='loss', min_delta=0.00001, patience=5) )

    model.compile(optimizer=Adam(lr=0.001), loss=dice_coef_loss_dc, metrics=[dice_coef_dc])
    ...
    np.save('data/detection_and_classification/history_with_augmentation.npy',history.history)
    ...
    acc = history.history['dice_coef_dc']
    val_acc = history.history['val_dice_coef_dc']
...
training_model_with_da()

def testing_model_augmentation():
    model = get_model_dc(128, 128, 4, 'data/detection_and_classification/weights_with_augmentation.h5')
    X_train, y_train, X_val, y_val, X_test, y_test = load_data()
    y_pred = model.predict(X_test, verbose=1, batch_size=128)

    check_predictions("test", y_pred, "detection_classification")
    ...
    # drawing some cases
    make_plots_none_sign(X_test, y_test, y_pred)
    make_plots_sign(X_test, y_test, y_pred)
