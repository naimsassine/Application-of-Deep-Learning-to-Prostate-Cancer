"""
This code contains different functions to preprocess the masks images for prostate and lesions
Here is the pipeline : 
- Resample the images so that they adapt to the spacing of the originals T2, with the same size
- Apply the affine registering technique to make sure they masks are aligned with the images
- Flip the masks to have them correctly aligned with the T2
...
def sum_lesions_to_pros_masks_beta() :
    # we have the significant + mask apart from the none-significant + mask
...
def concatenate_train_images(cases):
    ...
    for file_path in sorted(patient_files):
        if str(file_path).split("/")[8][:4] in cases : 
            image_3d = np.load(file_path)
            slices = cases[str(file_path).split("/")[8][:4]]
            for s in slices : 
...
def concatenate_train_masks():
    ...
    for file_path in sorted(patient_files):
        ...
        if len(np.unique(volume)) > 2:
            ...
            cases[str(file_path).split("/")[8][:4]] = slices
            for s in slices:
...
def cleaning_concatenated():
    # after running some tests with the U-net model I build, I realised that 
    # a lot of images didn't contain lesions, and that is because of the MRI technique
    # it captures a lot of images before and after the cancer appears.
    # that is why I built this function to clean the data a bit, and achieve better results, less overfitting
